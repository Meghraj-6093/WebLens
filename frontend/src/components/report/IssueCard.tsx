...
export const IssueCard: React.FC<IssueCardProps> = ({ issue }) => {
  ...
  const getSeverityBadge = () => {
    if (issue.passed) {
      return (
        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-500/10 text-[#34D399] border border-emerald-500/20">
    ...
    switch (issue.severity) {
      ...
      case 'high':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-[#FF6B35]/15 text-[#FF6B35] border border-[#FF6B35]/30">
            <AlertTriangle className="w-3 h-3" /> High
          </span>
        );
      case 'medium':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-[#FF804F]/15 text-[#FF804F] border border-[#FF804F]/30">
            <AlertCircle className="w-3 h-3" /> Medium
          </span>
        );
      case 'low':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-[rgba(243,240,232,0.08)] text-[#D8D4CA] border border-[rgba(243,240,232,0.14)]">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className="shrink-0 flex items-center gap-1.5">
              ...
              {!issue.passed && issue.scoreImpact !== undefined && issue.scoreImpact > 0 && (
                <span className="hidden xs:inline-flex px-1.5 py-0.5 rounded-md text-[10px] font-mono font-bold bg-[#FF6B35]/15 text-[#FF6B35] border border-[#FF6B35]/30">
                  -{issue.scoreImpact} pts
                </span>
              )}
            </div>
            <div className="min-w-0">
              <h4 className="text-xs sm:text-sm font-bold text-[#F3F0E8] tracking-tight truncate">
                {issue.title}
              </h4>
              <p className="text-[11px] text-[#8E8A82] truncate mt-0.5">
                {issue.description}
              </p>
            </div>
          </div>
        ...
        {isOpen && (
          <div className="px-4 pb-5 sm:px-5 sm:pb-5 pt-0 border-t border-[rgba(243,240,232,0.08)] space-y-4 text-xs">
            ...
            <div className="space-y-1 bg-[#0C0F14] p-3.5 rounded-xl border border-[rgba(243,240,232,0.08)]">
              <span className="font-bold text-[#D8D4CA] text-xs flex items-center gap-1.5">
                <AlertCircle className="w-3.5 h-3.5 text-[#FF6B35]" />
                Why this matters:
              </span>
              <p className="text-[#8E8A82] leading-relaxed pl-5">
                {issue.impact}
              </p>
            </div>
            ...
            <div className="space-y-1 bg-[#151A21] p-3.5 rounded-xl border border-[#FF6B35]/25">
              <span className="font-bold text-[#FF6B35] text-xs flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-[#FF6B35]" />
                How to resolve:
              </span>
              <p className="text-[#F3F0E8] leading-relaxed pl-5 font-mono text-[11px]">
